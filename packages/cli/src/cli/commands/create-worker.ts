...
export const createWorkerCommand = new Command('create-worker')
  ...
  .action(async (options: { name?: string }) => {
    ...
    try {
      ...
      await sessionManager.addJazzServerWorker(
        account.accountID,
        name,
        account.agentSecret
      );
      ...
      console.log(chalk.green(`✅ Created and stored Jazz Server Worker: ${name}`));
      console.log(chalk.gray(`Account ID: ${account.accountID}`));
      console.log(chalk.gray('You can now send messages using: roomy send'));
      process.exit(0);
    } catch (error) {
      console.error(chalk.red(`❌ Failed to create worker: ${(error as Error).message}`));
      process.exit(1);
    }
  });

<script lang="ts">
  ...
  // listable immediately; archived threads appear only once Phase 2 discovers
  // them and land at the end.
  ...
  const orderedBackfillRows = $derived.by(() => {
    ...
    // just found): channels first, then threads, each group in API order
    // (newest update first) — a thread never leads the list while a channel
    // is still pending. Threads nest under their parent entry when it has
    // one of its own.
